...
class SiakadModel extends Model
{
    ...
    {
        $builder = $this->db_siakad->table($table);
        if($select){
    ...
    {
        $builder = $this->db_siakad->table($table)->insert($data);
        if($lastQuery == true){
            var_dump ($this->db_siakad->getLastQuery()->getQuery());
            die();
        }
        if($builder){
            return $this->db_siakad->insertID();
        }
        return false;
    }
    ...
    {
        $builder = $this->db_siakad->table($table)->update($data, $where);
        if($lastQuery == true){
            var_dump ($this->db_siakad->getLastQuery()->getQuery());
            die();
        }
    ...
    {
        $builder = $this->db_siakad->table($table)->updateBatch($data, $where);
        if($lastQuery == true){
            var_dump ($this->db_siakad->getLastQuery()->getQuery());
            die();
        }
    ...
    {
        $builder = $this->db_siakad->table($table)->delete($where);
        if($lastQuery == true){
            var_dump ($this->db_siakad->getLastQuery()->getQuery());
            die();
        }
